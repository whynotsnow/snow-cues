# Snow Cues v2.0 Agent 入口

## 项目定位

Snow Cues 是一个纯前端、本地优先的密码派生与加密存储系统。应用必须可以作为静态站点部署到 GitHub Pages。

2.0 版本以用户显式维护的 `storageData` 文件夹作为唯一业务数据源。Syncthing、Git 同步和冲突自动合并不属于应用逻辑。不要在应用内实现本地目录监听、自动同步、自动合并、后端同步、账号系统或云端服务。
...
- `src/storage-data/`：2.0 `storageData` 文件格式、schema、hash、diff、repository 和文件夹访问。
- `src/storage-engine/`：存储类型、字段白名单和兼容导出层。
...
提交时直接读取本节记录的用户名和邮箱，并通过 `git -c user.name=... -c user.email=... commit ...` 使用它们。Commit message 必须遵循 Conventional Commits，例如 `feat: implement local password workspace`、`fix: handle decrypt failure state`、`docs: update project context`。
