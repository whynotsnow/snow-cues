# Snow Cues v1.0 项目上下文

## 项目定位

这是一个纯前端、本地优先的密码派生与加密存储系统。应用必须可以作为静态站点部署到 GitHub Pages，也可以由 Syncthing 等外部工具同步项目代码和构建产物。

Syncthing 不属于应用逻辑。不要在应用内实现本地目录同步、后端同步、账号系统或云端服务。
...
- IndexedDB 的密码条目只允许保存 `PasswordEntry` 中定义的字段。
...
- IndexedDB 的空间 profile 可以按 `spaceId` 保存全局初始化配置：`ruleChain` 和参与规则链的声明式导入规则 manifest。不要把这些字段写入单条 `PasswordEntry`。
- IndexedDB 的迁移批次和迁移条目必须与正式 `PasswordEntry` 分离。待迁移条目保存旧空间密文和非敏感元数据，不得直接出现在正式密码列表中。
- 密码输出必须先用 AES-GCM 加密，再进入 IndexedDB。
...
- `src/storage-engine/`：负责 IndexedDB CRUD 和存储字段白名单。
...
- `src/ui/hooks/useDetachedPasswordController.ts`：负责空间外游离密码表单、临时预览、复制状态和待迁入派生密钥内存状态；不得读写 IndexedDB。
...
- IndexedDB 数据库名是 `snow-cues`，版本号是 9，对象仓库包含 `password_entries`、`space_profiles`、`spaces`、`space_relations`、`migration_batches`、`migration_entries` 和 `password_groups`。升级时会把旧单空间数据迁移到 `default` 空间；若浏览器内已有同版本但对象仓库不完整的旧连接状态，打开数据库时会提升到下一内部版本补齐缺失仓库。
- `PasswordEntry` 当前字段为：`id`、`spaceId`、`encrypted_password`、`encrypted_memory_hint`、`groupId`、`platform`、`description`、`deprecatedAt`、`createdAt`、`updatedAt`。`spaceId` 是本地分区标识，不是派生输入，`groupId` 只是 UI 归属。
- `PasswordGroup` 当前字段为：`id`、`spaceId`、`name`、`description`、`outputPolicy`、`createdAt`、`updatedAt`。`outputPolicy` 是解密后的输出适配策略，不参与核心密码生成。
- `allowedStorageFields` 与 `sanitizePasswordEntry` 是存储边界的重要保护；新增字段前必须确认是否违反安全边界，并同步测试。
...
- `updatePasswordEntry` 不更新密码内容；条目废弃后禁止修改 `platform`，仍允许修改 `description` 和 `groupId`。
- `deletePasswordEntry` 在 storage-engine 中存在，但当前 UI 删除按钮是禁用的，不要在 UI 中启用删除，除非用户明确要求重新讨论。
- `space_profiles` 按 `spaceId` 保存空间级 profile：`spaceId`、`ruleChain`、`importedRuleManifests`、`createdAt`、`updatedAt`。这是空间初始化配置，不是密码条目字段。
- `migration_batches` 和 `migration_entries` 保存迁移队列。它们用于暂存旧空间密文、迁移进度和来源空间流转偏好，不是正式密码条目；迁移成功后才创建新的 `PasswordEntry`。

IndexedDB 维护经验：

- 新增对象仓库或索引必须提升 `DB_VERSION`，并在 `onupgradeneeded` 中幂等补齐所有必需 store/index。
- Chrome 可能保留“版本号已升级但对象仓库不完整”的开发期坏状态，常见表现是 `Failed to execute 'transaction' on 'IDBDatabase': One of the specified object stores was not found.`。
- 不能只在 UI hook 捕获这类错误；那只会把阻塞流程变成页面错误。根因修复必须在 `openDatabase()` 层完成 schema 校验和自愈升级。
- `openDatabase()` 成功后必须检查 `hasRequiredStores`。若缺 store，应关闭旧连接并用更高内部版本重新打开，触发升级流程补齐 schema。
- 新增 store 时同步补测试，模拟“当前版本数据库缺新 store”的坏状态，确认应用自动修复，不要求用户清空浏览器数据。
...
- 全局测试数据工具始终展示在页面底部，和是否进入空间无关；用于测试的按钮文案必须明确标注“测试”。空间外可删除指定空间或清空全部本地数据，空间内且策略允许时可清空当前空间密码数据或重置当前空间规则链配置。
...
- 不把派生输入或单条密码可重建派生输入的元数据写入 IndexedDB、localStorage、sessionStorage、URL 或文件。例外：空间级 profile 可以保存初始化后的全局 `ruleChain` 和参与规则链的声明式导入规则 manifest，密码条目可以保存非敏感 `spaceId` 用于本地分区，并可保存加密后的 `encrypted_memory_hint`。
