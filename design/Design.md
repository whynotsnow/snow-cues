...
- 左侧 `NavRail`：品牌、主导航、空间范围或当前空间状态，以及低干扰的产品资料外链入口。
...
产品介绍站和文档站入口应作为外部资料辅助存在，可在 `NavRail` 常驻展示，并在 Guidance 中以低优先级相关流程卡片提示。外链必须使用新标签页打开，不参与 hash 路由、不改变当前工作台状态，也不得被表述为业务数据、账号、同步或后端能力来源。
