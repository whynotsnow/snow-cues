...
describe("空间进入与门禁", () => {
  it("进入空间后可以用已存储密码和关键密钥进行校验", async () => {
    ...
    expect(
      within(getGuidancePanel()).getByText(
        /能解密既有密码，就代表本次空间主密码校验成功/
      )
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "前往空间主页" })
    ).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "校验后可新建" })).toBeDisabled();
    expect(
      screen.queryByRole("button", { name: "校验空间" })
    ...
    await waitFor(() =>
      expect(screen.getByText("解密失败，请检查关键密钥。")).toBeInTheDocument()
    );
    ...
    fireEvent.click(screen.getByRole("button", { name: "完成空间校验" }));
    await waitFor(() =>
      expect(screen.getByText("空间校验已完成。")).toBeInTheDocument()
    );
    fireEvent.click(screen.getByRole("button", { name: "密码管理" }));
    expect(
  ...
  it("空间主页允许改选用于校验的密码条目", async () => {
    ...
    await seedEncryptedPasswordEntry(
      "selectable",
      "entry-first",
      "First",
      "first-secret"
    ...
    expect(screen.getByLabelText("用于校验的密码条目")).toHaveValue(
      "entry-first"
    );
    await fillFirstSpaceMasterPassword();
    fireEvent.change(screen.getByLabelText("解密关键密钥"), {
  ...
  it("空间校验未完成时优先展示校验指引并阻塞迁移指引", async () => {
    ...
    const headings = within(guidancePanel)
      .getAllByRole("heading")
      .map((heading) => heading.textContent);
    expect(headings.at(0)).toBe("先完成空间校验");
    expect(
      within(guidancePanel).getByText(
        "当前空间校验未完成，完成校验后才能继续迁移。"
      )
    ).toBeInTheDocument();
  });
  ...
  it("已有密码的空间进入后必须先完成校验才能写入或修改", async () => {
    ...
    expect(
      screen.getByText("请先输入空间主密码并完成空间校验。")
    ).toBeInTheDocument();
  ...
  it("deprecated Space 禁止创建密码和修改规则，但保留历史查看入口", async () => {
    ...
    const historyPageNotice = within(
      screen.getByLabelText("当前页面")
    ).getByLabelText("页面通知");
    expect(
      within(historyPageNotice).getByText(/通常表示已有后继空间接替/)
    ).toBeInTheDocument();
    expect(
      within(historyPageNotice).getByText(
        /创建、编辑、废弃、规则变更和迁移管理均不可用/
      )
    ).toBeInTheDocument();
    expect(
      within(historyPageNotice).queryByText(/clone/)
    ).not.toBeInTheDocument();
    expect(
      within(getGuidancePanel()).getByText("可用操作")
    ).toBeInTheDocument();
    expect(
      within(getGuidancePanel()).getByRole("heading", {
        name: "历史空间可用操作"
    ...
    expect(
      within(getGuidancePanel()).queryByText("进入历史空间")
    ).not.toBeInTheDocument();
  ...
  it("历史空间有条目时显示解密准备指引而不是 active 空间校验指引", async () => {
    ...
    expect(
      within(getGuidancePanel()).getByText(/查看历史列表不需要校验/)
    ).toBeInTheDocument();
    expect(
      within(getGuidancePanel()).getByText(/历史密码校验区域输入空间主密码/)
    ).toBeInTheDocument();
    expect(
      within(getGuidancePanel()).queryByText(/设置本次空间主密码/)
    ).not.toBeInTheDocument();
    expect(
    ...
    expect(
      within(getGuidancePanel()).getByText(/本次会话已完成历史密码解密准备/)
    ).toBeInTheDocument();
  });
  ...
  it("archived Space 禁止创建、编辑、废弃、规则管理和日常派生", async () => {
    ...
    expect(
      within(getGuidancePanel()).getByText("受阻流程")
    ).toBeInTheDocument();
    expect(
      within(getGuidancePanel()).getByRole("heading", {
        name: "归档空间可用操作"
    ...
    expect(
      within(getGuidancePanel()).queryByText("进入归档空间")
    ).not.toBeInTheDocument();
    expect(screen.getByRole("button", { name: "新建不可用" })).toBeDisabled();
    fireEvent.click(screen.getByRole("button", { name: "规则管理" }));
    expectPageNotice("归档存储空间");
