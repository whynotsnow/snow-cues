...
describe("空间外索引与创建入口", () => {
  ...
  it("空间外点击新建空间后才展示创建方式", async () => {
    ...
    expect(screen.getByLabelText("创建方式")).toBeInTheDocument();
    expect(screen.getByRole("option", { name: "空白创建" })).toBeInTheDocument();
    expect(screen.getByRole("option", { name: "从已有的空间 clone 配置" })).toBeInTheDocument();
    expect(screen.getByRole("option", { name: "从已有的空间 clone 配置 + 密码迁移队列" })).toBeInTheDocument();
    expect(screen.getByRole("option", { name: "从导入文件创建空间" })).toBeInTheDocument();
    expect(screen.getByRole("option", { name: "从导入文件创建空间 + 密码迁移队列" })).toBeInTheDocument();
    ...
    await waitFor(() => expect(screen.getByText("已进入临时存储空间。在空间主页设置空间主密码后，可初始化规则链或创建密码。")).toBeInTheDocument());
    ...
    expect(within(getGuidancePanel()).getByRole("heading", { name: "初始化空间规则链" })).toBeInTheDocument();
