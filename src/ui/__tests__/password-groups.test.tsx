...
describe("密码组与输出适配", () => {
  it("可以创建密码组并在解密后进行密码输出适配", async () => {
    ...
    expect(screen.queryByRole("heading", { name: "密码组输出适配" })).not.toBeInTheDocument();
    ...
    await waitFor(() => expect(screen.getByText("密码组已创建。")).toBeInTheDocument());
    ...
    fireEvent.change(screen.getByLabelText("所属密码组，可选"), {
      target: { value: screen.getByRole("option", { name: "代码托管" }).getAttribute("value") ?? "" }
    });
    ...
    await waitFor(() => expect(screen.getByText("已解密核心密码")).toBeInTheDocument());
    await waitFor(() => expect(screen.getByText("密码输出适配")).toBeInTheDocument());
    expect(screen.getByText("适配密码")).toBeInTheDocument();
    expect(screen.queryByLabelText("常见策略预设")).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "保存为组输出策略" })).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "调整输出策略" }));
    expect(screen.getByLabelText("常见策略预设")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "保存为组输出策略" })).not.toBeDisabled();
    ...
    await waitFor(() => expect(screen.getByText("密码组输出适配已保存。")).toBeInTheDocument());
    fireEvent.click(screen.getByRole("button", { name: "输出适配" }));
