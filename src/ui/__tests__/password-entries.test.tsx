...
describe("密码条目管理", () => {
  it("同一时间只允许编辑一条密码，未保存修改会阻止切换", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /生成并保存/i }));
    await waitFor(() => expect(screen.getAllByRole("button", { name: "编辑条目" })).toHaveLength(2));
    ...
    await waitFor(() => expect(screen.getByText("当前条目有未保存修改，请先保存或取消。")).toBeInTheDocument());
  ...
  it("可以进入存储空间、生成、保存加密输出，并且不保存禁止的派生元数据", async () => {
    ...
    await waitFor(() => expect(screen.getByText("当前空间没有记录来源或接替关系。")).toBeInTheDocument());
    await expect(getSpace("default")).resolves.toBeNull();
    expect(screen.queryByRole("heading", { name: "迁移情况" })).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "密码管理" }));
    expect(screen.getByRole("heading", { name: "已存储密码" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "先初始化规则链" })).toBeInTheDocument();
    ...
    await waitFor(() => expect(screen.getByText("新建密码前需要先初始化存储空间规则链。")).toBeInTheDocument());
    await confirmRuleProfileWithMaster();
    ...
    await waitFor(() => expect(screen.getByText("请输入用于加密或解密的关键密钥。")).toBeInTheDocument());
    ...
    fireEvent.click(screen.getByRole("button", { name: "确认解密" }));
    await waitFor(() =>
      expect(screen.getByText("解密失败，请检查关键密钥。该条目保存了关键密钥记忆提示，你可以查看提示后重试。")).toBeInTheDocument()
    );
    fireEvent.click(screen.getByRole("button", { name: "查看记忆提示" }));
    await waitFor(() => expect(screen.getByText("蓝色杯子旁边的提示")).toBeInTheDocument());
    expect(screen.getByRole("button", { name: "隐藏记忆提示" })).toBeInTheDocument();
    ...
    fireEvent.click(screen.getByRole("button", { name: "确认解密" }));
    await waitFor(() => expect(screen.getByText(/仍然无法解密/)).toBeInTheDocument());
    ...
    await waitFor(() => expect(screen.getByLabelText("关键密钥记忆提示")).toHaveValue("蓝色杯子旁边的提示"));
    fireEvent.change(screen.getByLabelText("关键密钥记忆提示"), {
      target: { value: "" }
    });
    fireEvent.click(screen.getByRole("button", { name: "保存条目" }));
    await waitFor(() => expect(screen.getByText("条目信息已更新。")).toBeInTheDocument());
    fireEvent.click(screen.getByRole("button", { name: "查看记忆提示" }));
    await waitFor(() => expect(screen.getByText("蓝色杯子旁边的提示")).toBeInTheDocument());
    fireEvent.click(screen.getByRole("button", { name: "编辑条目" }));
    expect(screen.getByText("••••••••")).toBeInTheDocument();
    ...
    expect(screen.queryByLabelText("关键密钥记忆提示")).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "查看记忆提示" }));
    await waitFor(() => expect(screen.getByText("改成抽屉里的卡片")).toBeInTheDocument());
    expect(screen.queryByText("蓝色杯子旁边的提示")).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "编辑条目" }));
    fireEvent.click(screen.getByRole("button", { name: "解锁编辑" }));
    await waitFor(() => expect(screen.getByLabelText("关键密钥记忆提示")).toHaveValue("改成抽屉里的卡片"));
    fireEvent.click(screen.getByRole("button", { name: "清除提示" }));
    await waitFor(() => expect(screen.getByText("关键密钥记忆提示已清除。")).toBeInTheDocument());
    expect(screen.queryByRole("button", { name: "查看记忆提示" })).not.toBeInTheDocument();
    ...
    fireEvent.click(screen.getByRole("button", { name: "确认解密" }));
    await waitFor(() => expect(screen.queryByText("加密密码")).not.toBeInTheDocument());
    fireEvent.click(screen.getByRole("button", { name: "隐藏" }));
    await waitFor(() => expect(screen.getByRole("button", { name: "解密" })).toBeInTheDocument());
    fireEvent.click(screen.getByRole("button", { name: "解密" }));
    ...
    await waitFor(() => expect(screen.getByText("密码已标记为废弃。")).toBeInTheDocument());
    ...
    expect(screen.queryByRole("button", { name: "正在编辑" })).not.toBeInTheDocument();
    ...
    expect(screen.getByLabelText("导入声明式规则或规则数组")).toBeInTheDocument();
