    ...
    <section className="rules-section" aria-label="空间外工作台">
        ...
        <SectionHeader
          actions={
            <ActionGroup variant="tool">
              <Button onClick={() => void handleCreateStorageData()}>
                新建存储数据文件夹
              </Button>
              <Button onClick={() => void handleOpenStorageDataFolder()}>
                打开存储数据文件夹
              </Button>
            </ActionGroup>
          }
            ...
            <>
              <DescriptionList
                items={[
                  ...
                  {
                    label: "未保存状态",
                    value: storageDataDirty ? "有未保存改动" : "无未保存改动",
                  },
                ]}
              />
              {storageDataDownloadText ? (
      ...
      <Card aria-label="比较两个存储数据文件">
        <SectionHeader
          ...
          title={
            storageDataOpened ? "比较其他存储数据文件" : "比较两个存储数据文件"
          }
        />
        <div className="form-stack">
          <TextField
