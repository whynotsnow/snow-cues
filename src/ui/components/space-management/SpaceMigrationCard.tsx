import { formatMigrationBatchStatus, formatMigrationRuleSnapshot } from "../../displayLabels";
import { ActionGroup, Button, Card, CheckboxField, SectionHeader, SelectField, TextField } from "../../design-system";
...
export function SpaceMigrationCard({ controller, doneCount, pendingCount, totalCount }: SpaceMigrationCardProps) {
  const {
    migrationBatches,
    migrationEntries,
  ...
  const pendingMigrationEntries = migrationEntries.filter((entry) => entry.status === "pending");
  const allMigrationEntriesHandled = migrationEntries.length > 0 && pendingMigrationEntries.length === 0;
  ...
  const sourceVerificationEntry = pendingMigrationEntries.find((entry) => entry.id === sourceVerificationEntryId)
    ?? pendingMigrationEntries[0]
    ?? null;
  const batchIsDraft = selectedMigrationBatch?.status === "draft";
  const flowPreferenceLocked = Boolean(selectedMigrationBatch && (
  ...
  const migrationSummary = totalCount > 0
    ? allMigrationEntriesHandled
      ? "迁移条目已全部处理完成。"
  ...
  return (
    <Card aria-label="迁移情况">
      ...
      {migrationBatches.length > 0 ? (
        <SelectField label="迁移批次" onChange={(event) => setSelectedMigrationBatchId(event.target.value)} value={selectedMigrationBatchId}>
          {migrationBatches.map((batch) => (
            <option key={batch.id} value={batch.id}>
              {batch.sourceSpaceId} → {batch.targetSpaceId} · {formatMigrationBatchStatus(batch.status)}
            </option>
          ))}
        </SelectField>
      ) : null}
      {selectedMigrationBatch ? (
        <div className="effective-rule">
          <span>来源规则快照</span>
          <strong>{formatMigrationRuleSnapshot(selectedMigrationBatch.sourceProfileSnapshot)}</strong>
          <small>来源规则只读，用于理解旧空间；目标规则初始化后用于重新生成模式。</small>
        </div>
      ) : null}
      {selectedMigrationBatch ? (
        <div className="migration-flow-options">
          <CheckboxField
            ...
            <span>
              ...
              <small>默认开启。关闭后，全部条目迁移或跳过完成时不会自动创建接替关系，也不会自动把来源空间标记为历史空间。</small>
            </span>
            }
            onChange={(event) => void handleMigrationAutoFinalizeChange(event.target.checked)}
          />
          {flowPreferenceLocked && !selectedMigrationBatch.sourceFinalizedAt ? (
            <p className="field-note">迁移已开始或已完成，流转方式不能再修改。</p>
          ) : null}
          {selectedMigrationBatch.sourceFinalizedAt ? (
            <p className="login-note">来源空间状态已完成流转，并已记录接替关系。</p>
          ) : null}
      ...
      {!allMigrationEntriesHandled && selectedMigrationBatch && (sourceSessionVerified || sourceVerificationEntry) ? (
        <EntryVerificationPanel
          title="来源空间校验"
          description="选择一条旧空间密码，用旧空间主密码和这条密码的旧关键密钥完成来源空间校验。"
          entries={pendingMigrationEntries}
          selectedEntryId={sourceVerificationEntry?.id ?? sourceVerificationEntryId}
          ...
          feedback={sourceVerificationFeedback ? {
            tone: sourceVerificationFeedbackTone,
            title: sourceVerificationFeedbackTone === "error"
              ? "来源空间校验失败"
        ...
        >
          ...
          {sourceVerificationEntry ? (
            <TextField
              ...
              disabled={sourceSessionVerifying}
              label="旧关键密钥"
              onChange={(event) => setOldEntrySecrets((current) => ({ ...current, [sourceVerificationEntry.id]: event.target.value }))}
        ...
        <div className="entries-grid">
          {migrationEntries.map((entry) => (
            <MigrationEntryCard
              ...
              externalPasswordUpdated={Boolean(externalPasswordUpdated[entry.id])}
              setMode={(mode) => setMigrationModes((current) => ({ ...current, [entry.id]: mode }))}
              setOldEntrySecret={(value) => setOldEntrySecrets((current) => ({ ...current, [entry.id]: value }))}
              setNewEntrySecret={(value) => setNewEntrySecrets((current) => ({ ...current, [entry.id]: value }))}
              setReuseOldEntrySecret={(value) => setReuseOldEntrySecret((current) => ({ ...current, [entry.id]: value }))}
              setExternalPasswordUpdated={(value) => setExternalPasswordUpdated((current) => ({ ...current, [entry.id]: value }))}
            />
          ))}
        </div>
      ) : null}
      {lastMigratedPassword ? (
        <div className="memory-hint-box">
          ...
          <small>该明文只保存在当前界面状态中，离开空间或刷新后不会保留。</small>
        </div>
      ) : null}
    </Card>
  );
