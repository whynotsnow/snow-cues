    ...
    <Card aria-label="空间概览">
      ...
      <DescriptionList
        items={[
          ...
          { label: "空间状态", value: currentSpaceIsTemporary ? "临时空间，尚未持久化" : spaceStatusLabels[currentSpaceStatus] },
          { label: "规则链", value: ruleProfileConfirmed ? "已初始化" : "未初始化" }
        ]}
      />
      {currentSpaceIsTemporary ? (
        <div className="effective-rule">
          ...
          <small>初始化规则链或创建第一条密码后，才会持久保存空间 profile 和空间记录。</small>
        </div>
      ) : null}
      {spaceRelations.length > 0 ? (
        <div className="effective-rule">
          <span>空间指向关系</span>
          <strong>
            {spaceRelations.map((relation) => formatSpaceRelationLabel(relation.fromSpaceId, relation.toSpaceId, relation.type)).join("；")}
          </strong>
