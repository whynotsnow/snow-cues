...
export function SpaceOperationsCard({ controller }: SpaceOperationsCardProps) {
  ...
  const needsTargetSpace =
    spaceOperationMode === "clone_current_profile" ||
    spaceOperationMode === "clone_current_with_entries";
  const actionLabel =
    spaceOperationMode === "export_profile" ||
    spaceOperationMode === "export_full"
      ? "生成导出 JSON"
      : spaceOperationMode
        ? "创建并进入目标空间"
        : "执行当前空间操作";
    ...
    <Card aria-label="当前空间操作">
      ...
      <div className="form-stack">
        <SelectField
          ...
          onChange={(event) =>
            setSpaceOperationMode(
              event.target.value as typeof spaceOperationMode
            )
          }
          value={spaceOperationMode}
        >
          <option value="">选择当前空间操作</option>
          <option value="clone_current_profile">
            clone 当前空间配置到新的空间
          </option>
          <option value="clone_current_with_entries">
            从当前的空间 clone 配置 + 密码迁移队列
          </option>
        ...
        <ActionGroup variant="tool">
          <Button
            disabled={!spaceOperationMode}
            onClick={() => void handleCurrentSpaceOperation()}
          >
            {actionLabel}
          </Button>
        </ActionGroup>
