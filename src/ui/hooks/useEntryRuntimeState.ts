...
export function useEntryRuntimeState() {
  ...
  const [decryptingEntryId, setDecryptingEntryId] = useState<string | null>(
    null
  );
  const [decryptSpaceMasterPasswords, setDecryptSpaceMasterPasswords] =
    useState<Record<string, string>>({});
  const [decryptEntrySecrets, setDecryptEntrySecrets] = useState<
    Record<string, string>
  >({});
  const [visibleMemoryHints, setVisibleMemoryHints] = useState<
    Record<string, string>
  >({});
  const [viewedMemoryHintEntryIds, setViewedMemoryHintEntryIds] = useState<
    Record<string, boolean>
  >({});
  const [memoryHintLoadingEntryId, setMemoryHintLoadingEntryId] = useState<
    string | null
  >(null);
  const [editingMemoryHintEntryId, setEditingMemoryHintEntryId] = useState<
    string | null
  >(null);
  const [memoryHintEditValue, setMemoryHintEditValue] = useState("");
  const [memoryHintSavingEntryId, setMemoryHintSavingEntryId] = useState<
    string | null
  >(null);
  const [decryptLoadingEntryId, setDecryptLoadingEntryId] = useState<
    string | null
  >(null);
  const [editingEntryId, setEditingEntryId] = useState<string | null>(null);
  const [editingEntryDraft, setEditingEntryDraft] =
    useState<EditingEntryDraft | null>(null);
  const [editingEntryLoadingEntryId, setEditingEntryLoadingEntryId] = useState<
    string | null
  >(null);
  const [editingEntrySavingEntryId, setEditingEntrySavingEntryId] = useState<
    string | null
  >(null);
