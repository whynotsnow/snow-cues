...
export function diagnoseSpace(input: SpaceDiagnosticInput): SpaceDiagnosticStatus {
  ...
  if (input.entries?.some((entry) => entry.spaceId !== input.space?.spaceId || !entry.encrypted_password)) {
    return "corrupted";
  }
