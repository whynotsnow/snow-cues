...
describe("crypto-engine 加密", () => {
  it("可以完成 AES-GCM 加密密码的往返解密", async () => {
    ...
    await expect(decryptPassword(session.storageKey, encrypted)).resolves.toBe(
      "S3cure-Passw0rd"
    );
  });
  ...
  it("使用不同 storage key 解密会失败", async () => {
    ...
    await expect(
      decryptPassword(second.storageKey, encrypted)
    ).rejects.toThrow();
  });

  it("关键密钥参与条目加密 key 派生", async () => {
    const session = await createSession("master");
    const firstKey = await deriveRuntimeStorageKey(
      session.cryptoKey,
      "example.com:alice"
    );
    const secondKey = await deriveRuntimeStorageKey(
      session.cryptoKey,
      "example.com:bob"
    );
    const encrypted = await encryptPassword(firstKey, "S3cure-Passw0rd");

    await expect(decryptPassword(firstKey, encrypted)).resolves.toBe(
      "S3cure-Passw0rd"
    );
  ...
  it("游离密码使用派生密钥确定生成", async () => {
    const first = await generateDetachedPassword("temporary-key", {
      mode: "base62",
      maxLength: 24
    });
    const second = await generateDetachedPassword("temporary-key", {
      mode: "base62",
      maxLength: 24
    });
    const third = await generateDetachedPassword("another-key", {
      mode: "base62",
      maxLength: 24
    });
  ...
  it("游离密码拒绝空派生密钥和无效自定义字符集", async () => {
    await expect(
      generateDetachedPassword(" ", { mode: "base62", maxLength: 24 })
    ).rejects.toThrow("请输入派生密钥");
    await expect(
      generateDetachedPassword("temporary-key", {
        mode: "custom",
        customCharset: "aa",
        maxLength: 24
      })
    ).rejects.toThrow("自定义字符集至少需要包含两个不同字符");
  });
});
