...
import type { PasswordEntry, PasswordEntryInput, PasswordEntryPatch } from "./types";
...
export async function listPasswordEntriesBySpace(spaceId: string): Promise<PasswordEntry[]> {
...
export async function updatePasswordEntry(
  ...
  const existing = await requestToPromise<PasswordEntry | undefined>(store.get(id));
