...
describe("Snow Cues 应用冒烟流程", () => {
  it("可以完成空间主链路冒烟", async () => {
    ...
    await expect(getSpace("default")).resolves.toMatchObject({ status: "active" });
  ...
  it("批量导入声明式规则后可以确认初始化并返回密码管理", async () => {
    ...
    await waitFor(() => expect(screen.getByText("已导入 2 条规则，初始化前可加入规则链。")).toBeInTheDocument());
    fireEvent.click(screen.getAllByRole("button", { name: "删除规则" })[0]);
    expect(window.confirm).toHaveBeenCalledWith("确认删除这条导入规则？未初始化前删除不会影响已保存密码。");

    await confirmRuleProfileWithMaster();
    expect(window.confirm).toHaveBeenCalledWith("确认初始化规则链？初始化后本空间会冻结这组规则链，本次会话内不能再导入、停用、重命名或删除参与规则。");
