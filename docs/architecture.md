...
- `src/rule-registry/`：负责静态 pure function registry。导入规则通过 `imported-rule-algorithms.ts` 中的算法注册表映射到源码/构建期已注册的模板，算法模板负责声明式 `params` 的白名单校验与限幅；禁止 `eval`、`new Function`、动态脚本注入和运行时用户 JS。
...
应用外壳是左侧垂直导航轨道 + 右侧内容区的双栏布局：左侧 `NavRail` 承载品牌、页面导航和空间状态；右侧 `content-column` 内部分为可选的顶部存储数据固定栏和下方可滚动内容区（`content-scroll`），内容区内置右侧可收缩悬浮指引抽屉（`GuidanceDrawer`）。所有样式由 `src/styles.css` 手写提供，未引入 UI 组件库。
...
- `src/ui/guidance.ts`：根据顶层 controller 状态生成全局用户操作指引多卡片队列；只做状态推导，不持久化用户进度。
- `src/ui/pages/`：hash 路由页面入口，例如空间主页、规则管理、输出适配、密码管理和游离密码页面。
- `src/ui/components/`：垂直导航轨道、悬浮指引抽屉、空间主页卡片、密码管理、规则管理、条目卡片等展示组件。组件应尽量只渲染 props 和派发事件。
...
- `vite.config.ts` 引入 `vite-plugin-pwa` (`VitePWA` 插件)，配置 `strategies: "generateSW"`，使用 Workbox 自动生成 Service Worker。
- 更新模式为 `registerType: "prompt"`：新版 SW 在后台安装但不会自动激活，只有用户点击“立即刷新”才会 `skipWaiting` 并重新加载页面。这一设计保证进行中的解密 / 编辑会话绝不会被新版覆盖中断。
- `injectRegister: false`，Service Worker 注册由 `src/ui/hooks/usePwaUpdate.ts` 在应用代码中主动通过 `virtual:pwa-register` 虚拟模块加载和注册，便于精确控制生命周期并在测试中安全降级。

## PWA 与 Service Worker

### 缓存边界

Service Worker 由 Workbox 生成，采用 `generateSW` 策略，只在 `workbox.globPatterns` 中缓存静态应用外壳资源：JS、CSS、HTML、字体及图标。**用户的 `storageData` 永远不会进入 SW 的 Cache API**。`storageData` 始终由 File System Access API 按需读写，驻留在用户本机磁盘上，SW 无法触及。

### 离线行为

- SW 通过 `navigateFallback: "index.html"` 实现 SPA 离线导航回退。断网时用户可加载应用外壳，在桌面 / Android 上继续通过 File System Access API 访问 `storageData`。
- iOS Safari 不支持 File System Access API，离线时仅能打开应用外壳页面，无法访问本地 `storageData` 文件夹。
- 离线缓存不包含用户业务数据、密钥或密码密文。

### 更新提示

- `src/ui/hooks/usePwaUpdate.ts`：封装 SW prompt 注册逻辑。仅在 `navigator.serviceWorker` 可用时注册；在 jsdom/测试环境下安全降级，`needRefresh` 始终为 `false`。
- `src/ui/components/PwaUpdateBanner.tsx`：全局 PWA 更新横幅，挂在 `src/App.tsx` 顶层，跨所有路由可见。`prompt` 模式下会展示“检测到新版本可用。”及“立即刷新”/“稍后”按钮。另有一次性离线就绪 toast“应用已可离线使用。”。
- 该组件自管状态，不依赖 `AppController`，也不复用 `useSystemNotifications`（后者在浏览器通知权限授予时会委托系统级 Notification，不适合版本更新提示）。
...
- Vitest 不加载 `vite-plugin-pwa` 插件，`virtual:pwa-register` 虚拟模块不可解析。`vitest.config.ts` 通过 `resolve.alias` 将其映射到 `src/__mocks__/virtual-pwa-register.js` 的 mock 实现（返回空注册函数）。
- `PwaUpdateBanner` 通过可选的 `pwaUpdateState` 和 `registerFn` props 支持依赖注入，使测试在不依赖真实 SW 的前提下覆盖渲染、按钮回调和降级路径。
